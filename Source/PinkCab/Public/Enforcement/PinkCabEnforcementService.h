...
class FPinkCabEnforcementService
{
public:
        ...
        FPinkCabEnforcementSettlement& OutSettlement)
    {
        ...
        const FPinkCabEconomyTransaction Fine = FPinkCabEconomyTransaction::Debit(
    ...
    static FPinkCabTransactionId MakeFineTransactionId(const FPinkCabStableId& EventId)
    {
        if (!EventId.IsValid())
        {
            return FPinkCabTransactionId();
        }
        return FPinkCabTransactionId(PinkCabWorldId::StableToken(TEXT("fine:"), EventId.Serialize()));
    }

private:
    static bool IsViolation(const FPinkCabRoadRuleDefinition& Rule, const FPinkCabRoadObservation& Observation)
    {
        switch (Rule.Kind)
        {
        ...
        const FPinkCabRoadObservation& Observation)
    {
        ...
        const double ExcessRatio = Rule.SpeedLimitCmPerSec > 0.0
            ? (Observation.SpeedCmPerSec - Rule.SpeedLimitCmPerSec) / Rule.SpeedLimitCmPerSec
            : 0.0;
        return FMath::Clamp(
            Rule.BaseSeverity + static_cast<float>(FMath::Max(0.0, ExcessRatio) * 0.5),
            0.0f, 1.0f);
    }
        ...
        const FPinkCabRoadObservation& Observation)
    {
        ...
        return FPinkCabStableId(PinkCabWorldId::StableToken(TEXT("event:"), Payload));
    }
};

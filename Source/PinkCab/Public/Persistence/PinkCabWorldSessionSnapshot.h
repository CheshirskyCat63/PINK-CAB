...
public:
        ...
        FPinkCabCityDeltaSnapshot& OutSnapshot)
    {
        ...
        for (const FString& Key : Keys)
        {
            ...
            Snapshot.Records.Add({Record.DeltaId, Record.Kind, Record.SubjectStableId, Record.OperationKey});
        }
    ...
    {
        if (Snapshot.SchemaVersion != FPinkCabStateKernelSnapshot::CurrentSchemaVersion)
        {
            return false;
        }
        ...
        FPinkCabWorkdaySessionState& OutState)
    {
        ...
        if (!FPinkCabWorkdaySessionState::TryCreate(
                WorkdayId, Snapshot.Ordinal, Snapshot.ElapsedGameSeconds,
                Snapshot.MaxReplayJournalEntries, Restored))
        {
            return false;
        }
...
private:
    static bool ValidateCityDeltas(const FPinkCabCityDeltaSnapshot& Snapshot)
    {
        if (Snapshot.SchemaVersion != FPinkCabCityDeltaSnapshot::CurrentSchemaVersion
            || Snapshot.MaxDeltas <= 0 || Snapshot.Records.Num() > Snapshot.MaxDeltas)
        {
            return false;
        }
    ...
    static bool ValidateWorkday(const FPinkCabWorkdaySessionSnapshot& Snapshot)
    {
        if (Snapshot.SchemaVersion != FPinkCabWorkdaySessionSnapshot::CurrentSchemaVersion
            || Snapshot.Ordinal < 0
            ...
            || Snapshot.AppliedHouseholdOperationIds.Num() > Snapshot.MaxReplayJournalEntries
            || Snapshot.HouseholdTransactionCount != Snapshot.AppliedHouseholdOperationIds.Num())
        {
            return false;
        }

        FPinkCabStableId ParsedWorkday;
        if (!FPinkCabStableId::TryParse(Snapshot.WorkdayId, ParsedWorkday))
        {
            return false;
        }
        ...
        for (const FString& Id : Snapshot.AppliedHouseholdOperationIds)
        {
            ...
            if (!FPinkCabStableId::TryParse(Id, ParsedId) || UniqueIds.Contains(ParsedId.Serialize()))
            {
                return false;
            }

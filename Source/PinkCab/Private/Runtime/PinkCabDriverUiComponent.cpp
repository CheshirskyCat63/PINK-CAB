...
TSharedRef<SWidget> UPinkCabDriverUiComponent::BuildTelemetryPanel()
{
    ...
    return SNew(SBorder).Padding(FMargin(16,12)).BorderBackgroundColor(FLinearColor(0.015f,0.018f,0.022f,0.88f))
        [ SNew(SVerticalBox)
          ...
          + SVerticalBox::Slot().AutoHeight().Padding(0,7,0,0)[ SNew(STextBlock).Font(FCoreStyle::GetDefaultFontStyle(TEXT("Regular"),9)).ColorAndOpacity(FLinearColor(1,1,1,0.58f)).Text(FText::FromString(TEXT("MOUSE STEER   SPACE LOOK   Q CLUTCH   Q+WHEEL CLUTCH RELEASE   W+WHEEL BRAKE   E+WHEEL THROTTLE\n1 SIGNALS   2 HORN   3 GEARBOX   4 HANDBRAKE   RMB GRIP / HOLD   LMB ACTION / MOVE LEVER"))) ] ];
}
    ...
    SystemMenuOverlay = SNew(SOverlay)
        ...
        [ SNew(SBorder).Padding(FMargin(30.0f, 26.0f)).BorderBackgroundColor(FLinearColor(0.025f, 0.025f, 0.035f, 0.97f))
          [ SNew(SVerticalBox)
            ...
            + SVerticalBox::Slot().AutoHeight().Padding(0,18,0,0)[ SNew(SBorder).Padding(FMargin(14,12)).BorderBackgroundColor(FLinearColor(1,1,1,0.055f)).Visibility_Lambda([WeakThis]() { return WeakThis.IsValid() && WeakThis->bSystemMenuSettingsOpen ? EVisibility::Visible : EVisibility::Collapsed; })[ SNew(SVerticalBox) + SVerticalBox::Slot().AutoHeight()[ SNew(STextBlock).Text(FText::FromString(TEXT("SETTINGS / CONTROLS"))).Font(FCoreStyle::GetDefaultFontStyle(TEXT("Bold"),13)).ColorAndOpacity(Pink) ] + SVerticalBox::Slot().AutoHeight().Padding(0,6,0,0)[ SNew(STextBlock).Text(FText::FromString(TEXT("MOUSE STEER   SPACE LOOK\nQ CLUTCH   Q+WHEEL CLUTCH RELEASE\nW+WHEEL BRAKE   E+WHEEL THROTTLE\n1 SIGNALS   2 HORN   3 GEARBOX   4 HANDBRAKE\nRMB GRIP / HOLD   LMB ACTION / MOVE LEVER\nESC MENU"))).Font(FCoreStyle::GetDefaultFontStyle(TEXT("Regular"),11)).ColorAndOpacity(FLinearColor(1,1,1,0.78f)) ] ] ]

...
bool FPinkCabRecoveryOrchestrator::ApplyTerminalRecovery(
    FPinkCabGamePersistenceOwners& Owners)
{
    ...
    {
        const EPinkCabFareLoopState State =
            Owners.Fare.GetState();
        if ((State == EPinkCabFareLoopState::Active
                || State
                    == EPinkCabFareLoopState::AwaitingPayment)
            && !Owners.Fare.FailForTerminalRecovery(
                Owners.Load))
        {
            return false;
        }
...
bool FPinkCabRecoveryOrchestrator::AdvanceToNextWorkday(
    ...
    FPinkCabGamePersistenceOwners& Owners)
{
    ...
    FPinkCabWorkdaySessionState NextWorkday =
        Owners.Workday;
    if (!NextWorkday.ResetForNextWorkday(
            NextWorkdayId,
            NextOrdinal))
    {
        return false;

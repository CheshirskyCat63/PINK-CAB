...
bool ConfigureStraightRoad(AMetaRoad& Road, FAutomationTestBase& Test)
{
    ...
    {
        Test.TestEqual(TEXT("nine authored surfaces on left side"), Layout.Sections[0].Left.Num(), 9);
        Test.TestEqual(TEXT("nine authored surfaces on right side"), Layout.Sections[0].Right.Num(), 9);
    }
...
bool FPinkCabGenerateL1EndlessMetaRoadAssets::RunTest(const FString& Parameters)
{
    ...
    Settings->GenerationMode = EMetaRoadAssetGenerationBehavior::AutoGenerateAndAutosave;

...
bool FPinkCabTrafficProxyCandidate::IsValid() const
{
...
void FPinkCabTrafficInteractionBubbleResult::Reset()
{
    Assignments.Reset();
    DemotedTrafficIds.Reset();
}

FPinkCabTrafficInteractionBubble::FPinkCabTrafficInteractionBubble(int32 InCapacity)
    : Capacity(FMath::Max(1, InCapacity))
{
}

bool FPinkCabTrafficInteractionBubble::Update(
    ...
    FPinkCabTrafficInteractionBubbleResult& OutResult)
{
    ...
    Sorted.Sort([](
        const FPinkCabTrafficProxyCandidate& A,
        const FPinkCabTrafficProxyCandidate& B)
    {
...
int32 FPinkCabTrafficInteractionBubble::NumAssigned() const
{
    return AssignedSlots.Num();
}

...
struct FPinkCabWorldChunkCandidate
{
...
struct FPinkCabWorldMaterializationSettings
{
    ...
    bool IsValid() const
    {
        return MaxMaterializedChunks > 0 && MaxDistanceSquared >= 0
            && AllowedLayers.Num() > 0 && MaxAnchorRequests >= 0;
    }
};

struct FPinkCabWorldMaterializationResult
{
...
class FPinkCabWorldMaterializationPolicy
{
public:
        ...
        FPinkCabWorldMaterializationResult& OutResult)
    {
        ...
        {
            if (!Candidate.IsValid() || !Settings.AllowedLayers.Contains(Candidate.Coord.Layer))
            {
                continue;
            }

            const int64 DX = static_cast<int64>(Candidate.Coord.Longitudinal) - PlayerCoord.Longitudinal;
            const int64 DY = static_cast<int64>(Candidate.Coord.Lateral) - PlayerCoord.Lateral;
            const int64 DistanceSquared = DX * DX + DY * DY;
            if (DistanceSquared > Settings.MaxDistanceSquared)
            {
                continue;
            }
        ...
        OutResult.RequestSignature = PinkCabWorldId::StableToken(TEXT("materialize:"), SignaturePayload);
        return true;
    }

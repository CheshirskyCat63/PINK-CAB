...
bool FPinkCabL1RouteHook::IsValid() const
{
    return !HookId.IsEmpty() && LaneId.IsValid()
        && !ContactId.IsEmpty() && RouteOrder >= 0;
}

FPinkCabL1RouteHookRuntime::FPinkCabL1RouteHookRuntime(int32 InMaxHooks)
    : MaxHooks(FMath::Max(1, InMaxHooks))
{
}

FString FPinkCabL1RouteHookRuntime::MakeHookId(
    const FPinkCabVerticalContactRecord& Contact,
    int32 RouteOrder)
{
    if (!Contact.IsValid() || RouteOrder < 0) return FString();
...
bool FPinkCabL1RouteHookRuntime::TryAddHook(
    ...
    FString& OutHookId)
{
    ...
    if (HookId.IsEmpty() || Hooks.Contains(HookId)) return false;
...
FString FPinkCabL1RouteHookRuntime::GetReconstructionSignature() const
{
    TArray<FString> Keys;
    Hooks.GetKeys(Keys);
    Keys.Sort();
...
int32 FPinkCabL1RouteHookRuntime::Num() const { return Hooks.Num(); }

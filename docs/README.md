...
1. [`AUTHORITY.yaml`](AUTHORITY.yaml) вЂ” machine-readable current authority/scope graph.
2. [`PINK_CAB_ACTIVE_BASELINE.md`](PINK_CAB_ACTIVE_BASELINE.md) вЂ” concise active product baseline.
3. [`FIRST_EURO_SCOPE.md`](FIRST_EURO_SCOPE.md) вЂ” exact first-12-month single-player scope.
4. [`PINK_CAB_BASE100_CODE_ARCHITECTURE.md`](PINK_CAB_BASE100_CODE_ARCHITECTURE.md) вЂ” modular implementation doctrine.
5. [`PINK_CAB_BASE100_TECH_OWNER_PACK_01.md`](PINK_CAB_BASE100_TECH_OWNER_PACK_01.md) вЂ” current code-only owner-question intake.
6. [`PINK_CAB_START90_READINESS.md`](PINK_CAB_START90_READINESS.md) вЂ” current reproducible START-90 score and gap.
...
Technical Owner Pack 01: **196 rows = 99 LOCKED + 3 CALIBRATION + 72 PROPOSED DEFAULT + 22 OPEN**.

Scoring is reproducible: LOCKED/CALIBRATION = 1.0; PROPOSED DEFAULT = 0.5; OPEN = 0.0.

**Current code-only specification readiness = 70.4%. START-90 FAIL. Gap = 19.6 percentage points.**
...
If all 72 proposed defaults are owner-accepted, score becomes 174/196 = 88.8%; at least 3 of the 22 OPEN rows must then close to cross START-90 at 177/196 = 90.31%. Historical в‰€59% and 38.2% scores are retired.
...
- `10977288` Vehicle Technology Stack;

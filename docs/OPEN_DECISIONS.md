...
FIRST EURO keeps a generic `RecoveryPolicy/RecoveryHook` boundary. Pack `I06` is LOCKED: terminal/immobilizing Tatra fails any active fare, ends the Workday, commits an atomic save and recovers the still-damaged car to Repair next Workday with no free reset; fee settlement still obeys separate Economy policy.
...
## Owner lock batch · 2026-09-11 · runtime/release/input foundation

Owner accepted `A03`, `A10`, `A13`, `A14`, `C08`. Locked outcomes: eight shipping runtime modules (`Core / Vehicle / Taxi / World / Traffic / Economy / Persistence / Interaction`) with optional non-shipping Editor/Tests; GitHub Actions on self-hosted Windows Unreal runner with fail-closed production merge; Windows 10/11 x64 + DX12 + Steam FIRST EURO support; accepted 1080p60 Low minimum / 1440p60 High recommended hardware-performance seed with 16.67 ms frame target; semantic KBM rebinding with conflicts + Restore Defaults and no physical-control-model change.
...
A02 LOCKED: authoritative gameplay/state/interaction code is C++-first as far as practical; Blueprint is used where appropriate for thin composition, visual authoring and contextual glue. C06 remains LOCKED but its older universal LMB ATTENTION / RMB GO wording is superseded by Recovery Input Contract R1: hold Space for gaze/target search; 1–4 quick-recall saved physical targets without actuation; RMB acquires/retains Gearbox/Handbrake without moving it or stealing steering; RMB+LMB manipulates the lever; releasing LMB returns mouse XY to steering while RMB may remain held; LMB press/hold remains for authored momentary controls; mouse wheel owns detent/rotary/incremental controls where supported. Target selection/recall/grip never implicitly actuates.

...
| C-02 | OPEN | Existing ControllerThrottleDrive still contains direct fixture setup. Replace with a drivetrain input-path test that does not directly move gearbox/handbrake. |
...
| C-04 | OPEN | Need one automated PlayerController→cockpit→runtime→Chaos→movement path for drivetrain controls, with any unavoidable fixture setup named explicitly. |
...
| C-08 | OPEN | Movement acceptance must stop bypassing the exact cockpit states suspected in owner failure. |
...
| C-11 | OPEN | Unit/provider sign checks exist; signed live vehicle right-turn evidence still missing. |

...
require_text "$registry" "status_counts: {locked: 44, calibration: 3, proposed_default: 88, open: 61}"
require_text "$registry" "numerator_points: 91.0"
require_text "$registry" "start90_percent: 46.4"
require_text "$registry" "start90_gate_passed: false"
require_text "$registry" "gap_to_start90_percentage_points: 43.6"
require_text "$registry" "all_proposed_defaults_accepted_percent: 68.9"
require_text "$registry" "open_rows_needed_after_all_defaults_accepted_to_cross_90: 42"
...
# Exact current 61-row OPEN owner set.
...
if [ "$actual_open" != "$expected_open" ]; then
  ...
  printf '%s\n' "$actual_open" >&2
  fail "normalized 61-row OPEN owner set drifted"
fi
require_text "$registry" "count: 61"
...
require_text docs/PINK_CAB_START90_READINESS.md "91 / 196 = 46.4%"
require_text docs/PINK_CAB_START90_READINESS.md "42 of the remaining 61 OPEN rows"
...
require_text docs/OPEN_DECISIONS.md "61 genuine OPEN owner rows"
